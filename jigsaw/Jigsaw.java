...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		// *************************************
		// 访问节点数大于90000个则认为搜索失败
		int maxNodesNum = 90000;  
		...
		while (this.openList.isEmpty() != true && searchedNodesNum <= maxNodesNum) {	
			...
			// (2-3)寻找所有与currentJNode邻接且未曾被访问的节点，将它们全部插入到openList中
	...
	/**（Demo+实验二）计算并修改状态节点jNode的代价估计值:f(n)=e1+e2+e3。
	 * @param jNode - 要计算代价估计值的节点；此函数会改变该节点的estimatedValue属性值。
	 */
	private void estimateValue(JigsawNode jNode) {
		jNode.setEstimatedValue(estimateValue1(jNode) * 5 + estimateValue2(jNode) * 10 + estimateValue3(jNode)*10 );
	}

	// 后续节点不正确的数码个数
	public static int estimateValue1(JigsawNode jNode) {
		int s = 0; 
